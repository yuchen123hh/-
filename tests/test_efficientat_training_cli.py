...
class EfficientATTrainingCliTests(unittest.TestCase):
    def test_dry_run_writes_training_plan_without_cuda(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self._write_manifest(train_manifest, [("a.wav", "cough")])
            self._write_manifest(val_manifest, [("b.wav", "smoke_alarm")])
            output_dir = base / "run"
    ...
    def _write_manifest(self, path: Path, rows: list[tuple[str, str]]) -> None:
        with path.open("w", encoding="utf-8", newline="") as handle:
            ...
            writer.writeheader()
            for audio_path, label in rows:
                writer.writerow({"audio_path": audio_path, "label": label})
